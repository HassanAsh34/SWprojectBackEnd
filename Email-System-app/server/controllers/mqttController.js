...
function createClient() {
    ...
    client.on('message', async (topic, message) => {
      ...
      try {
        ...
        // console.log(Email)
        await folder.updateOne({userEmail:jsonMessage.sender,name:"send"},{$push:{emailsArray:Email._id}})
        for (const recipuent of Email.recipients) {
            await folder.updateOne({userEmail:recipuent,name:"inbox"},{$push:{emailsArray:Email._id}})
        }
      } catch (error) {
        console.log(error);
      }
